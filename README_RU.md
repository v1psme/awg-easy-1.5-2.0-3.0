...
- **AWG 2.0** — добавляет S3, S4, I1–I5 CPS (сигнатуры инициирующих пакетов); профиль мимикрии через `MIMICRY_PROFILE=dns|tls|quic|sip` (по умолчанию `dns`)
...
- **API Ключ** — Bearer token для интеграции внешних сервисов (управление клиентами, аплинки, ACL, DNS)
...
| `I_R_MIN` | int | Мин. значение `<r>` для DNS-пакетов (по умолчанию: 2) |
| `I_R_MAX` | int | Макс. значение `<r>` для DNS-пакетов (по умолчанию: 40 — см. Известные Проблемы) |
| `I1_DNS_SITES` | string | Список доменов через запятую для dns-профиля (по умолчанию: `icloud.com,google.com,nvidia.com`) |
| `I_DNS_MIMIC_ALL` | bool | Alias dns-профиля: I2–I5 тоже в DNS-формате (по умолчанию `false`) |
...
### Профили Мимикрии (`MIMICRY_PROFILE`)

I1–I5 — **декои**: их отправляет инициатор рукопожатия, принимающая сторона их НЕ валидирует.
Must-match у сервера и клиента — только **S1–S4, H1–H4, HeaderProtectionKey**. Смена профиля
применяется при рестарте (миграция в WireGuard.js): существующие клиенты продолжают работать,
переэкспорт конфигов нужен только чтобы применить новый профиль декоя клиентам.
...
| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/api/api-key` | Получить API ключ (требуется сессия) |
| `POST` | `/api/api-key/regen` | Перегенерировать API ключ (требуется сессия) |
...
| **Крупные I-пакеты** | ⚠️ Задокументировано | QUIC I1 ~1200 байт, Firefox I1 512 байт: pumbaX-опыт — «mobile AWG does not always deliver (especially I5)». При проблемах на мобильных — `MIMICRY_ONLY_I1=true` или профиль `dns` |
